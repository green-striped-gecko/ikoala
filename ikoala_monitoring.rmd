...
params:
  location_table: newlocations.csv
  repname: ikoala.docx
  resultsfolder: results
  studysite: Coastal
  tree_list_table: newtreelist.csv
  tree_table: newtrees.csv
  site_visits: newsitevisits.csv
  workdir: empty
  grid350: 0
  grid500: 1
  grid1000: 0
...
# set some parameters for things here....
plotjitteramount=0.001
...
# 
# params <- list()
# params$workdir <- "C://Aaron//Koala//revised ikoala files//"
# params$location_table <- "newlocations.csv"  #name of location csv file (LOCATION_MASTER in the access data base)
# params$tree_list_table <-"newtreelist.csv"  #name of tree activity table (TREE_MASTER in the access data base)
# params$tree_table <- "newtrees.csv" #name of tree list csv file (TREESPECIES_ID in the access data base)
# params$site_visits <- "newsitevisits.csv" # list of site visits
# params$resultsfolder <- "results57" #name of the results folder where the output is stored
# params$studysite <- "Coastal"   # filter that is applied for the subset of locations (based on column study_site in the LOCATION_MASTER table)
# params$grid350 <- 0 # filter that is applied for the subset of location based on the grid being examined
# params$grid500 <- 0 # filter that is applied for the subset of location based on the grid being examined
# params$grid1000 <- 1 # filter that is applied for the subset of location based on the grid being examined
...
site_visit_table <- params$site_visits
...
locationsALL<-read.csv(paste0(workfolder,"/",location_table),stringsAsFactors = FALSE)
trees<-read.csv(paste0(workfolder,"/",tree_table),stringsAsFactors = FALSE)
treelist<-read.csv(paste0(workfolder,"/",tree_list_table),stringsAsFactors = FALSE)
sitevisits<-read.csv(paste0(workfolder,"/",site_visit_table),stringsAsFactors = FALSE)
...
# first file is locationsALL

# first make everything uppercase to simplify the process of cleaning up data files...
...
# Have to change the IDs to make them clearer across tables...
colnames(locationsALL)[grep("^ID$",names(locationsALL))]<-"SiteID"

# the following columns have been moved elsewhere (sitevisits)...
# colnames(locationsALL)[grep("VISITED.SITE",names(locationsALL))]<-"VISITED SITE"  # this column got moved to a new file
#colnames(locationsALL)[grep("STUDY.SITE",names(locationsALL))]<-"STUDY SITE" # this column got moved to a new file
#colnames(locationsALL)[grep("STUDY.SITE",names(locationsALL))]<-"StudySite" # this column got moved to a new file
#colnames(locationsALL)[grep("DATE.SURVEYED",names(locationsALL))]<-"Date" # this column got moved to a new file

# keep only cells on a grid matching the one we're looking at
if (length(gridfil)>0) locationsALL<-locationsALL[locationsALL$GRID %in% gridfil,]
locationsALL<-locationsALL[,c("SiteID","Planned_Easting","Planned_Northing","LATITUDE","LONGITUDE","GRID")]



# second file is sitevisits

# first simplify by making all column headers uppercase...
colnames(sitevisits)<-toupper(colnames(sitevisits))

# next rename the column headers to be consistent with the headers that we've previously used...
colnames(sitevisits)[grep("VISITED.SITE",names(sitevisits))]<-"VISITED SITE" 
colnames(sitevisits)[grep("STUDY.AREA",names(sitevisits))]<-"StudySite" # note that in the most recent version, they've changed from STUDY.SITE to STUDY.AREA
colnames(sitevisits)[grep("DATE.SURVEYED",names(sitevisits))]<-"Date" # this column got moved to a new file

# Have to change the IDs to make them clearer across tables...
colnames(sitevisits)[grep("LOCATION.ID",names(sitevisits))]<-"SiteID"
colnames(sitevisits)[grep("^ID$",names(sitevisits))]<-"VisitID"

# first get rid of all the sites that haven't been visited
sitevisits<-sitevisits[sitevisits$"VISITED SITE"==1,]
# now get rid of all sites that aren't within the region of interest
vlocations<-sitevisits[sitevisits$StudySite==region,]
...
vlocations<-vlocations[,c("VisitID","SiteID","S_EASTINGS","S_NORTHINGS","A_LEVEL","VISITED SITE","Date","ANNUM","TREESWITHSCATS","StudySite","GEOLOGY","SOILTYPE")]



# third file is trees

# first change column name for consistency with other tables....
colnames(trees)[grep("VISIT_ID",names(trees))]<-"VisitID"

# Clean up trees
trees<-trees[,c("VisitID","TREE_NUM","SPECIES_ID","DBH","STEM_NUM","KFP_CONFIRMED")]


# Don't need this as the locationsALL table already has lats and longs...
# however, will change them to "Long" and "Lat" in place of Longitude and Latitude
...
# vlocations$Planned_Easting<-latslongs$X1
# vlocations$Planned_Northing<-latslongs$X2
# names(vlocations)[names(vlocations) == 'Planned_Easting'] <- 'Long'
# names(vlocations)[names(vlocations) == 'Planned_Northing'] <- 'Lat'
names(locationsALL)[names(locationsALL) == "LONGITUDE"] <- "long"
names(locationsALL)[names(locationsALL) == "LATITUDE"] <- "lat"



# fourth file is treelist

# Clean up the treelist data set
names(treelist)[grep("^ID$",names(treelist))]<-"SPECIES_ID"
treelist<-treelist[,c("Species","SPECIES_ID","Scientific_name","Common_name")]
treelist<-treelist[treelist$Species!="Blank", ] #remove species ID blank


# trap to make sure that we haven't accidentally ditched all of our sites

if(length(locationsALL$Planned_Easting)<1 || length(sitevisits$VisitID)<1) stop("Please check the parameters (Study site and Grids) you are using. The filtered dataset contains no observations.")



# begin combining datasets
trees <- merge(trees, treelist)
#add vlocations to trees (including treelist already)
treesvlocation_int <- merge(trees, vlocations)
#add site details to treesvlocation_int
treesvlocation<-merge(treesvlocation_int,locationsALL)
...
activity.sites <-ddply(treesvlocation, .(SiteID,VisitID), summarize, siteactive=max(KFP_CONFIRMED), Lat=mean(lat), Long=mean(long),Year=mean(ANNUM))
...
latmin<-min(activity.sites$Lat[activity.sites$SiteID!=338718],na.rm=TRUE) # need to 
latmax<-max(activity.sites$Lat[activity.sites$SiteID!=338718],na.rm=TRUE)
longmin<-min(activity.sites$Long, na.rm=TRUE)
longmax<-max(activity.sites$Long, na.rm=TRUE)
longmin<-min(activity.sites$Long[activity.sites$SiteID!=338718],na.rm=TRUE)
longmax<-max(activity.sites$Long[activity.sites$SiteID!=338718],na.rm=TRUE)
...
```{r sitemap, fig.cap=figRef("sitemap","Study area. Circles indicate all potential sampling points in the study area" )  ,echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=5,fig.height=5}
#############################################
...
PlotOnStaticMap(newmap,lat = jitter(activity.sites$Lat,amount=plotjitteramount), pch = 16, lon = jitter(activity.sites$Long,amount=plotjitteramount),  cex=activity.sites$mapdotsize,  col=ifelse(activity.sites$siteactive==1,colblue,colred), FUN = points, add=FALSE) # have added jitter in case a site is visited more than once...



# Create a shapefile of the points...
points.activity.sites<-SpatialPointsDataFrame(activity.sites[,c("Long","Lat")],activity.sites[,c("SiteID","VisitID","siteactive","Year")])
...
PlotOnStaticMap(newmap,lat = jitter(activity.sites$Lat,amount=plotjitteramount), pch = 16, lon = jitter(activity.sites$Long, amount=plotjitteramount),  cex=activity.sites$mapdotsize,  col=ifelse(activity.sites$siteactive==1,colblue,colred), FUN = points, add=FALSE)
dev.off()
```
...
numtreesactsites <- dim(trees[trees$VisitID %in% activesites$VisitID,])[1]
proptreesactsites<-round((numtreesactsites/numtrees*100), digits=0)
numtreesinactsites <- dim(trees[trees$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==0] ,])[1]
...
## 2.2) Estimated occupancy and probability of detection
...
```{r calculate detectability, echo=FALSE, results='hide', message=FALSE, warning=FALSE}

# start after getting the treesvlocation file created in ikoala.rmd....

visitlist<-as.numeric(names(table(treesvlocation$VisitID)))

trunctrees<-treesvlocation[,c("VisitID","TREE_NUM","KFP_CONFIRMED")]

threevisits<-matrix(NA,nrow=length(visitlist),ncol=3)
for(i in 1:length(visitlist)){
  cursite<-visitlist[i]
  tmpsite<-trunctrees[trunctrees$VisitID==visitlist[i],]
  if(dim(tmpsite)[1]==30){ # ignoring sites where they had something other than 30 trees
    rans<-rank(runif(30,0,1))
    tmpsitesrank<-cbind(tmpsite,rans)
    startrnk3<-1
    finrnk3<-10
    for(k in 1:3){
      threevisits[i,k]<-max(tmpsitesrank[((tmpsitesrank$rans>=startrnk3) & (tmpsitesrank$rans<=finrnk3)),"KFP_CONFIRMED"])
      startrnk3<-startrnk3+10
      finrnk3<-finrnk3+10
    }
  } else {
    for (k in 1:3){
     rans<-rank(runif(dim(tmpsite)[1],0,1))
     tmpsitesrank<-cbind(tmpsite,rans)
     threevisits[i,k]<-max(tmpsitesrank[((tmpsitesrank$rans>=1) & (tmpsitesrank$rans<=10)),"KFP_CONFIRMED"])
    } 
  }
}
...
"
# Bundle data
win.data <- list(y = threevisits, R = nrow(threevisits), T = ncol(threevisits))
# Initial values
zst <- apply(threevisits, 1, max) # Observed occurrence as starting values for z
inits <- function() list(z = zst)

# Parameters monitored
paramsocc <- c("psi", "p", "p30", "occ.fs")
# MCMC settings
ni <- 1200
nt <- 2
nb <- 200
...
out <- R2jags::jags(win.data, inits, paramsocc, textConnection(modelstring), n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
# Summarize posteriors
print(out, dig = 2)
occupancy_results<-out$BUGSoutput$summary

```

The mean estimate for the probability of detection, *p*, was: `r round(occupancy_results[4,1],2)` with 95% confidence intervals of (`r round(occupancy_results[4,3],2)` to `r round(occupancy_results[4,7],2)`). 

The mean estimate of the percentage of occupied sites accounting for imperfect detection, *psi*, was: `r round(occupancy_results[5,1]*100,2)` with 95%  confidence intervals of (`r round(occupancy_results[5,3]*100,2)` to `r round(occupancy_results[5,7]*100,2)`)


## 2.3) Tree species composition of active and inactive sites
...
write.csv(treecntbyspeciesABC, row.names = F, file=paste(workfolder,"/",resultsfolder,"/tables/Treelist.csv",sep=""), na="")
...
siteactivity<-ddply(treesvlocation,.(VisitID),sitesum01=sum(KFP_CONFIRMED), sumtreessites=sum(counter), Lat=mean(lat), Long=mean(long), summarize)
...
# plot points first
PlotOnStaticMap(activemap, lat = jitter(siteactivity$Lat,amount=plotjitteramount), lon = jitter(siteactivity$Long,amount=plotjitteramount), pch = 16, col =cols[ceiling(siteactivity$sitesum01/3)+1], add = FALSE, FUN = points, cex=0.6)
...
points.site.activity.levels<-SpatialPointsDataFrame(siteactivity[,c("Long","Lat")],siteactivity[,c("VisitID","site.activity","levels")])
...
acttreesvlocation<-treesvlocation[treesvlocation$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==1],]
...
sitespecificSR<-ddply(acttreesvlocation,.(VisitID,Species),sumspeciessite=sum(counter),sitespeciessum01=sum(KFP_CONFIRMED), summarize)
...
inactsites<-treesvlocation[treesvlocation$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==0] ,]
#summary of active sites
actsites<-treesvlocation[treesvlocation$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==1] ,]
#number of inactive sites each species is present at
Ninactsites<-ddply(inactsites,.(Species), summarize, Ninactsites=length(unique(VisitID))) 
#number of active sites each species is present at 
Nactsites<-ddply(actsites,.(Species), summarize, Nactsites=length(unique(VisitID))) 
...
activonly.treesites<-treesvlocation[treesvlocation$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==1],]
...
red_treelist<-treelist[,c("Species","SPECIES_ID","Scientific_name")]
...
activity.sites <-ddply(treesvlocation, .(VisitID), summarize, siteactive=max(KFP_CONFIRMED))
acttreesvlocation<-treesvlocation[treesvlocation$VisitID %in% activity.sites$VisitID[activity.sites$siteactive==1],]
...
colnames(tt)<- c(paste("Period",1:nperiods))
tt <- addmargins(tt)
kable(tt, caption = tabRef("sumocc","Summary of occurences of koalas per study period"))
...
rownames(tt)<- c("absent","present")
colnames(tt)<- c(paste("Period",1:nperiods))
kable(tt, caption = tabRef("sumocc2","Percentage of occurrences of koalas per study period"))

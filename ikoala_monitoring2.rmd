...
author: "Bernd Gruber & Aaron Adamack Version 0.33"
date: '`r Sys.Date()`'
output:
  word_document:
    fig_caption: yes
...
params:
  report_name: ikoala report Numeralla
  database: d:/bernd/r/koalaoccupancy/GDA94z55Database2017JUN.accdb
  study_periods: d:/bernd/r/ikoala/data/periods_st.csv
  study_site: d:/bernd/r/ikoala/data/numeralla.csv
  restricted: yes
  results_folder: d:/bernd/r/ikoala/results
  max_distance: 501
...
```{r}
#This version of ikoala_monitoring.rmd can be run via a GUI ikoala_monitoring_GUI.r. In case you do not want to run it with the GUI you need to specify the input parameters above 
```
...
*  max_distance:  `r params$max_distance`
*  resticted: `r params$restricted`
...
```{r, results="asis"}
if (nperiods==1) cat("**The input contains only one defined study period, therefore a detailed ikoala report for that period will be produced, but no occupancy modelling output .** \n")

if (nperiods>1) cat("**More than one study period is defined, therefore a detailed report for all sites combinded over all periods and a monitoring ikoala report are produced. For the monitoring report study sites will be projected/moved to sites provided via the study_site file, if they are within the specified in max_distance.** \n")


if (params$restricted) cat(paste0("\n As restricted is switched to **yes**, the detailed report is based on sites as defined by the file:", params$study_site," over all periods. \n"))

if (!params$restricted) cat(paste0("\n As restricted is switched to **no**, the detailed report is based on sites defined by the entry:", defperiods$region[1]," in the data base.\n "))
...
occdata <- function(filter, loctrees, from=NA, to=NA, nJ=5 )
  
{
study<-loctrees[loctrees$STUDY_AREA==filter ,]
study <- study[!is.na(study$SITE_VISIT_ID),]  #get rid of NAs in VISIT_ID
if (from) study <- study <- study[as.numeric(substr(study$DATE_SURVEYED,1,4)>=from) & as.numeric(substr(study$DATE_SURVEYED,1,4)<=to),]
...
for (i in 1:nperiods)
{  
per1occ <-occdata(filter = as.character(defperiods$region[i]),loctrees = loctrees, from=defperiods$from[i], to =defperiods$to[i],  nJ = nJ ) 
koac1 <- per1occ
koac1$period <- i
...
if (params$restricted & !is.null(coresites))
{
...
This is an automatically generated report of koala activity and occurrence for a specified sampling period and region. The terminology and analyses performed within this report are consistent with earlier reports by Phillips & Callaghan (2000), Allen et al. (2009), and Biolink (2011), Gruber & Adamack 2016, 2017).

The report entails the following output:
  
  * Description of sampling area and periods
  
  * Occurrence - determined by the presence or absence of koala faecal pellets at a sampling site for every period
  * Activity - the proportion of trees with one or more koala faecal pellets at a sampling site for every period.
  * Preference for certain tree species (referred to as 'strike rate') - defined as the probability for every period
that a faecal pellet is found under a tree of a particular tree species

  * If more than one period is defined an analysis on the trend of occurrence between periods is run. The analysis uses a bayesian occupancy modeling approach as detailed in Gruber & Adamack (2016, 2017).

(For more information on terminology, sampling methods and analysis, please, refer to the above mentioned reports)

#2. Study area and sampling periods
...
Data collection at each sampling site consisted of searching the leaf litter within 1 m of the trunks of 30 trees at each sampling site for the presence of koala faecal pellets, identifying each tree's species and measuring its diameter at breast height (DBH). This report is for the `r params$studysite` study area that has an extent that ranges from `r round(latmin,2)` to `r round(latmax,2)` latitude and from `r round(lonmin,2)` to `r round(lonmax,2)` longitude. 


```{r sitemap, fig.cap=figRef("sitemap","Study area and sampling sites." )  ,echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=5,fig.height=5}
#############################################
# Create a map of the study area #
#############################################
newmap <- GetMap.bbox(lonR=c(lonmin, lonmax), latR=c(latmin, latmax), maptype="terrain") 

PlotOnStaticMap(newmap,lat = sites$lat, pch = 21, lon = sites$lon,  cex=1,  col="white", FUN = points, add=FALSE, bg="black") 
# Create a shapefile of the points...
points.sites<-SpatialPointsDataFrame(coords=sites[,c("lon","lat")], sites[, c("x","y")])
writeOGR(points.sites,file.path(params$results_folder,"sites"),overwrite_layer=TRUE, delete_dsn=TRUE,layer="sites", driver="ESRI Shapefile")


```

```{r printsitemap, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
tiff(file.path(params$results_folder,"studysite.png"),width = 5, height=5, res = 300,units = "in")

index <- which(points.sites$activitykoalas==1)
#PlotOnStaticMap(newmap)
PlotOnStaticMap(newmap,lat = as.numeric(points.sites$lat[index]), lon = as.numeric(points.sites$lon[index]), col="blue", FUN = points, add=FALSE, pch=16)
dev.off()
```



#3.) Occurrences of koalas in the study area

```{r sitemappa, fig.cap=figRef("sitemappa","Presence/absence map of the study area." ) , fig.subcap=c("one","tow") ,echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=5,fig.height=5}
#############################################
# Create a map of the study area #
#############################################
...
# 4.) Results - Activity of koalas in the study area
...
filter <- as.character(defperiods$region[1])
from <- min(defperiods$from)
to <- max(defperiods$to)

study<-loctrees[loctrees$STUDY_AREA==filter ,]
study <- study[!is.na(study$SITE_VISIT_ID),]  #get rid of NAs in VISIT_ID
if (from) study <- study <- study[as.numeric(substr(study$DATE_SURVEYED,1,4)>=from) & as.numeric(substr(study$DATE_SURVEYED,1,4)<=to),]

if (params$restricted) study <- study[study$SITE_VISIT_ID %in% sites$PLOT_ID,]
...
png(file.path(params$results_folder,"koalaselection.png",sep=""),width = 10, height=8, res = 300,units = "in")
p<-ggplot(tempmelt, aes(species, delta,fill=factor(select)))+geom_boxplot()+scale_fill_manual(name="Selection by koalas",values=c("1"="blue","0"="yellow","-1"="red"),labels=c("1"="Positive","0"="Neutral","-1"="Negative"),breaks=c("1","0","-1"))+xlab("Tree species")+ylab("Observed - simulated strike rate")
print(
    p + geom_boxplot()+theme_classic()+theme(axis.text.x = element_text(size=10,color="black",angle=-90,vjust=0.5,hjust=0),axis.text.y = element_text(size=10),axis.title = element_text(size=14),title=element_text(size=14))+geom_hline(yintercept=0.00)+ylim(-0.5,0.5)+ theme(legend.position=c(.92, .85))
  )
...
write.csv(lmdetails,file.path(params$results_folder,"Linear_regression.csv",sep=""),row.names=F)
...
png(file.path(params$results_folder,"sizeSRspecies.png",sep=""),width = 8, height=6, res = 300,units = "in")
SpeciesDBHplot<-qplot(x=sizeclass, y=probability,data=DBHspecificSR)
print(SpeciesDBHplot +stat_smooth(method="lm",se=FALSE,aes(weight=sumspeciesDBH)) +facet_wrap(~Species, scales="free_x")+theme_classic()+xlab("Size class")+ylab("Strike rate")+theme(axis.text.x = element_text(size=10,color="black"),axis.text.y = element_text(size=04,color="black"),axis.title = element_text(size=14),title=element_text(size=14)))
dev.off()
```
...
## 5.1) Monitoring periods and summary tables
...
```{r sitemapcore2, fig.cap=figRef("sitemap32",paste0("Monitoring area. The monitoring area is defined by the sites listed in file: ", params$study_site," and consists of ",nrow(coresites)," sites." ) ) ,echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=5,fig.height=5}
if (nperiods>1)
{
#############################################
# Create a map of the study area #
#############################################
newmap <- GetMap.bbox(lonR=c(lonmin, lonmax), latR=c(latmin, latmax), maptype="terrain") 
PlotOnStaticMap(newmap,lat = coresites$lat, pch = 21, lon = coresites$lon,  cex=1,  col="white", FUN = points, add=FALSE, bg="black") 
# Create a shapefile of the points...
...
```{r sitemap_per, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width=5,fig.height=5}
#############################################
# Create a map of the study area #
#############################################
if (nperiods>1)
...
cols <- c("red","blue","darkgrey")


for (i in 1:nperiods)
{
  
PlotOnStaticMap(newmap,lat = coresites$lat, pch = 16, lon = coresites$lon,  cex=1,  col="darkgrey", FUN = points, add=FALSE) 
legend("bottomleft", legend = c("absent","present"), pch=16, title = paste("Period",i), col=cols)
   
set.caption(paste0("Presence/absence map of the study area for period ",i,"." ) )
PlotOnStaticMap(newmap,lat = sites$lat[sites$period==i], pch = 21, lon = sites$lon[sites$period==i],  cex=1,  col="white", FUN = points, add=TRUE, bg=cols[sites$activitykoalas[sites$period==i]+1]) 
legend("bottomleft", legend = c("absent","present","not monitored"), pch=16, title = paste("Period",i), col=cols)
...
## 5.3 Estimated occupancy over time and probability of detection
...
out <- R2jags::jags.parallel(data=win.data, inits=list(inits()), parameters=parameters,model.file =  "./data/Dynocc.txt", n.chains = 3, n.thin =2, n.iter = 10000, n.burnin = 500)
...
tab.param <- c(paste0("occupancy rate[",1:nperiods,"]"),paste0("# occupied sites[",1:nperiods,"]"), "detection probability at a site (30 trees)", paste0("colonisation rate[",1:(nperiods-1),"-", 2:nperiods,"]"), paste0("extinction rate[",1:(nperiods-1),"-", 2:nperiods,"]"), paste0("probability of increase[",1:(nperiods-1),"-", 2:nperiods,"]"))

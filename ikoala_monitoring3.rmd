...
author: "Bernd Gruber & Aaron Adamack - Version 0.44"
date: '`r Sys.Date()`'
output:
  word_document:
    fig_caption: yes
...
params:
  ...
  database: database/GDA94z55Database2017JUN.accdb
  study_periods: periods/periods_ST.csv
  study_site: sites/sites_numeralla.csv
  restricted: site
...
```{r, results="asis"}
if (nperiods==1) cat("**The input contains only one defined study period, therefore a detailed ikoala report for that period will be created, but no monitoring report.** \n")

if (nperiods>1) cat("An *ikoala* monitoring analysis is included because input from more than one survey period is specified in the study periods file.  \n")


if (params$restricted=="site") cat(paste0("\n As restricted is switched to **site**, the detailed report is based on sites as defined by the file:", params$study_site," over all periods. \n"))

if (!params$restricted=="region") cat(paste0("\n As restricted is switched to **region**, the detailed report is based on sites defined by the entry:", defperiods$region[1]," in the periods file (",params$study_periods,").\n "))

cat(paste0("Sites are included and moved to the next possible grid location if they occur within the specified maximum distance (",params$max_distance,"m) of the grid used in the monitoring survey."))
...
if (params$restricted=="site" & !is.null(coresites))
{
...
#newmap <- GetMap.bbox(lonR=c(lonmin, lonmax), latR=c(latmin, latmax), maptype="terrain") 
  ...
  ###intermediate solutions
plot(coresites$lon, coresites$lat, asp=1, pch=16) 
...
The following figures show the grid-sites that were assessed in each monitoring period and thier presenece/absence results.
...
###intermediate solutions
plot(coresites$lon, coresites$lat, asp=1, pch=16, col="darkgrey")   
...
plot(sites$lon[sites$period==i], sites$lat[sites$period==i], asp=1, pch=16, col=cols[sites$activitykoalas[sites$period==i]+1])   
...
if (params$restricted=="site") study <- study[study$SITE_VISIT_ID %in% sites$PLOT_ID,]
